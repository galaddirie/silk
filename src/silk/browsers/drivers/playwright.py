...
"""
from typing import Dict, List, Optional, Any, Union, cast, Tuple, TypeVar, Type
...
from silk.models.browser import (
    BrowserOptions, ClickOptions, TypeOptions, MouseMoveOptions, DragOptions,
    NavigationOptions, WaitOptions, MouseButtonLiteral, WaitStateLiteral, 
    NavigationWaitLiteral, CoordinateType, KeyPressOptions
)

logger = logging.getLogger(__name__)

# Type alias for a tuple of (context_id, playwright_context)
ContextEntry = Tuple[str, PlaywrightContext]
# Type alias for a tuple of (page_id, playwright_page)
...
class PlaywrightElementHandle(ElementHandle[PlaywrightNativeElement]):
    ...
    async def get_bounding_box(self) -> Result[Dict[str, float], Exception]:
        ...
        try:
            ...
            if box is None:
                return Error(Exception("Element is not visible or has been removed from DOM"))
            return Ok({
                "x": box["x"],
                "y": box["y"],
                "width": box["width"],
    ...
    async def fill(self, text: str, options: Optional[TypeOptions] = None) -> Result[None, Exception]:
    ...
    async def select(self, value: Optional[str] = None, text: Optional[str] = None) -> Result[None, Exception]:
    ...
    async def get_parent(self) -> Result[Optional[ElementHandle], Exception]:
        ...
        try:
            ...
            # Convert the parent handle to a PlaywrightElementHandle
            parent_element = cast(PlaywrightElementHandle, parent)
            return Ok(PlaywrightElementHandle(
                driver=self.driver,
    ...
    async def get_children(self) -> Result[List[ElementHandle], Exception]:
        ...
        try:
            children = await self.element_ref.evaluate_handle("el => Array.from(el.children)")
            ...
            for i in range(len(children_array)):
                ...
                result_children.append(PlaywrightElementHandle(
                    driver=self.driver,
                    page_id=self.page_id,
    ...
    async def query_selector(self, selector: str) -> Result[Optional[ElementHandle], Exception]:
    ...
    async def query_selector_all(self, selector: str) -> Result[List[ElementHandle], Exception]:
        ...
        try:
            ...
            for element in elements:
                result_elements.append(PlaywrightElementHandle(
                    driver=self.driver,
                    page_id=self.page_id,
                    element_ref=element,
                    selector=selector
...
class PlaywrightDriver(BrowserDriver[Playwright]):
    ...
    async def launch(self) -> Result[None, Exception]:
        ...
        try:
            ...
            # Choose the browser type based on options
    ...
    async def create_context(self, options: Optional[Dict[str, Any]] = None) -> Result[str, Exception]:
        ...
        try:
            ...
            # Apply context-specific options if provided
            if options:
        ...
        try:
            ...
            if options:
                ...
                if options.wait_until:
                    # Convert NavigationWaitLiteral to Playwright's LoadState
    ...
    async def screenshot(self, page_id: str, path: Optional[Path] = None) -> Result[Union[Path, bytes], Exception]:
        ...
        try:
            ...
            if path:
                ...
                # No path specified, return the image as bytes
    ...
    async def query_selector(self, page_id: str, selector: str) -> Result[Optional[ElementHandle], Exception]:
        ...
        try:
            ...
            # Element is a PlaywrightNativeElement, which is the correct type for element_ref
            return Ok(PlaywrightElementHandle(
                driver=self,
                page_id=page_id,
                element_ref=element,
    ...
    async def query_selector_all(self, page_id: str, selector: str) -> Result[List[ElementHandle], Exception]:
        ...
        try:
            ...
            for element in elements:
                result.append(PlaywrightElementHandle(
                    driver=self,
                    page_id=page_id,
                    element_ref=element,
                    selector=selector
    ...
    async def wait_for_selector(self, page_id: str, selector: str, options: Optional[WaitOptions] = None) -> Result[Optional[ElementHandle], Exception]:
        ...
        try:
            ...
            if options:
                ...
                if options.state:
                    # Convert WaitStateLiteral to Playwright's WaitForSelectorState
    ...
    async def click(self, page_id: str, selector: str, options: Optional[ClickOptions] = None) -> Result[None, Exception]:
        ...
        try:
            ...
            if options:
                if options.button:
                    # Convert MouseButtonLiteral to Playwright's MouseButton
    ...
    async def double_click(self, page_id: str, selector: str, options: Optional[ClickOptions] = None) -> Result[None, Exception]:
        ...
        try:
            ...
            # Set click_count to 2 for double click
    ...
    async def mouse_drag(self, context_id: str, source: Union[str, ElementHandle, CoordinateType], target: Union[str, ElementHandle, CoordinateType], options: Optional[DragOptions] = None) -> Result[None, Exception]:
        ...
        try:
            ...
            # Set default options
            force = options.force if options and options.force is not None else False
    ...
    async def get_element_text(self, page_id: str, element: ElementHandle) -> Result[str, Exception]:
        ...
        try:
            # Check if this is our wrapper class
    ...
    async def extract_table(self, page_id: str, table_element: ElementHandle, include_headers: bool = True, header_selector: str = "th", row_selector: str = "tr", cell_selector: str = "td") -> Result[List[Dict[str, str]], Exception]:
        ...
        try:
            if not isinstance(table_element, PlaywrightElementHandle):
                return Error(Exception("Table element is not a PlaywrightElementHandle"))
            
            page = self.pages.get(page_id)
            if not page:
                return Error(Exception(f"Page with ID '{page_id}' not found"))
            
            # Get header cells if needed
            headers = []
            if include_headers:
                # Query header cells
                header_elements_result = await table_element.query_selector_all(header_selector)
            ...
            for row in rows:
                ...
                # Skip header row if it doesn't contain data cells
                if len(cells) == 0:
                    continue
    ...
    async def _get_coordinates(self, page: Page, target: Union[str, ElementHandle, CoordinateType]) -> Tuple[Optional[int], Optional[int]]:
        """
