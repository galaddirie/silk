...
class Query(Action[Optional[ElementHandle]]):
    ...
    async def execute(self, context: ActionContext) -> Result[Optional[ElementHandle], Exception]:
        ...
        try:
            ...
            if isinstance(self.selector, SelectorGroup):
                ...
                selector = self.selector if isinstance(self.selector, Selector) else Selector(
                    type=SelectorType.CSS, value=self.selector
                )
    ...
    async def _query_selector(self, selector: Selector, page: BrowserPage) -> Result[Optional[ElementHandle], Exception]:
...
class QueryAll(Action[List[ElementHandle]]):
    ...
    async def execute(self, context: ActionContext) -> Result[List[ElementHandle], Exception]:
    ...
    async def _query_selector_all(self, selector: Selector, page: BrowserPage) -> Result[List[ElementHandle], Exception]:
        """Helper method to query a specific selector"""
        try:
...
class GetText(Action[str]):
    ...
    async def execute(self, context: ActionContext) -> Result[str, Exception]:
        ...
        try:
            ...
            if isinstance(self.selector, SelectorGroup):
                ...
                return Error(Exception(f"Failed to get text from any selector in group: {self.selector.name}"))
            else:
                selector = self.selector if isinstance(self.selector, Selector) else Selector(
                    type=SelectorType.CSS, value=self.selector
                )
    ...
    async def _get_text_from_selector(self, selector: Selector, page: BrowserPage) -> Result[str, Exception]:
...
class GetAttribute(Action[Optional[str]]):
    ...
    async def execute(self, context: ActionContext) -> Result[Optional[str], Exception]:
        ...
        try:
            ...
            if isinstance(self.selector, SelectorGroup):
                ...
                return Error(Exception(f"Failed to get attribute from any selector in group: {self.selector.name}"))
            else:
    ...
    async def _get_attribute_from_selector(self, selector: Selector, page: BrowserPage) -> Result[Optional[str], Exception]:
...
class GetHtml(Action[str]):
    ...
    async def execute(self, context: ActionContext) -> Result[str, Exception]:
        ...
        try:
            logger.debug(f"Getting {'outer' if self.outer else 'inner'}HTML from element with selector {self.selector_desc}")
            
            if isinstance(self.selector, ElementHandle):
                # Use element's get_html method directly
...
class GetInnerText(Action[str]):
    ...
    async def execute(self, context: ActionContext) -> Result[str, Exception]:
        ...
        try:
            ...
            if isinstance(self.selector, ElementHandle):
                # Use element's get_inner_text method directly
...
class ExtractTable(Action[List[Dict[str, str]]]):
    ...
    async def execute(self, context: ActionContext) -> Result[List[Dict[str, str]], Exception]:
        ...
        try:
            ...
            if isinstance(self.table_selector, SelectorGroup):
                ...
                for selector in self.table_selector.selectors:
                    ...
                    # Use the driver's extract_table method
                ...
                table_element = table_element_result.default_value(None)
                if table_element is None:
                    return Error(Exception(f"Table element not found: {self.selector_desc}"))
