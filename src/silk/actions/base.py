from typing import TypeVar, Generic, Callable, Any, Dict, List, Optional, Protocol, Awaitable, Union, overload, cast, ParamSpec
...
class Action(Generic[T]):
    ...
    def __init__(self, name: str, description: Optional[str] = None):
        ...
        self.description_option = Some(description) if description is not None else Nothing
        self.description = self.description_option.default_value("")  # Default to empty string if None
    ...
    def and_then(self, f: Callable[[T], 'Action[S]']) -> 'Action[S]':
        ...
        class ChainedAction(Action[S]):
            ...
            async def execute(self, driver: BrowserDriver) -> Result[S, Exception]:
                # Use the monadic bind pattern from Result
                result = await original_action.execute(driver)
                
                # Properly handle the railway pattern
                try:
                    # Only bind/continue if we have a success result
                    return await result.bind(
    ...
    def __rshift__(self, other: Union[Callable[[T], S], 'Action[Any]']) -> 'Action[Any]':
        """
    ...
    def __or__(self, other: 'Action[S]') -> 'Action[Union[T, S]]':
        ...
        class FallbackAction(Action[Union[T, S]]):
            ...
            async def execute(self, driver: BrowserDriver) -> Result[Union[T, S], Exception]:
                try:
    ...
    def __and__(self, other: 'Action[S]') -> 'Action[tuple[T, S]]':
        ...
        return parallel(self, other).map(lambda results: (results[0], results[1]))
...
def create_action(name: str, execute_fn: Callable[[BrowserDriver], Awaitable[Result[T, Exception]]], 
                  description: Optional[str] = None) -> Action[T]:
    """
