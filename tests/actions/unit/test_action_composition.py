...
from silk.actions.composition import (
    sequence, parallel, pipe, fallback, compose
)
...
class TestActionBaseComposition:
    ...
    @pytest.mark.asyncio
    async def test_and_then_success(self, action_context, mock_browser_page, mock_element_handle):
    ...
    async def test_retry_all_failures(self, action_context):
        ...
        assert result.is_error()
        assert "Always fails" in str(result.error) or "All 3 attempts failed" in str(result.error)
    ...
    @pytest.mark.asyncio
    async def test_pipe_operator_with_action(self, action_context, mock_browser_page, mock_element_handle):
        """Test the >> operator for action sequencing."""
        # Setup mock behaviors - properly handle element selection
        mock_browser_page.query_selector.side_effect = lambda selector: Ok(mock_element_handle)
    ...
    @pytest.mark.asyncio
    async def test_fallback_operator(self, action_context, mock_browser_page, mock_element_handle):
    ...
    @pytest.mark.asyncio
    async def test_parallel_operator(self, action_context, mock_browser_manager, mock_browser_context, mock_browser_page):
        ...
        # Verify parallel execution created separate contexts
        assert mock_browser_manager.create_context.call_count == 2
        # Update expectation to match actual behavior - get_page() is called 4 times
        assert mock_browser_context.get_page.call_count == 4

        # Contexts should be closed when done
    ...
    @pytest.mark.asyncio
    async def test_executable_action(self, mock_browser_manager, mock_browser_context, mock_browser_page):
...
class TestSequence:
    ...
    async def test_sequence_all_success(self, action_context):
        ...
        assert list(block) == ["result3", "result2", "result1"]  # Results are collected in a Block
...
class TestParallel:
    @pytest.mark.asyncio
    async def test_parallel_all_success(self, action_context, mock_browser_manager, mock_browser_context, mock_browser_page):
        ...
        block = result.default_value(None)
        assert isinstance(block, Block)
        assert set(block) == {"result1", "result2", "result3"}  # Order is not guaranteed in parallel
    ...
    @pytest.mark.asyncio
    async def test_parallel_one_fails(self, action_context, mock_browser_manager, mock_browser_context, mock_browser_page):
    ...
    async def test_parallel_no_browser_manager(self, action_context):
        ...
        # Verify
        assert result.is_error()
        assert "Cannot execute parallel actions without a browser manager" in str(result.error)
...
class TestPipe:
    ...
    async def test_pipe_first_action_fails(self, action_context):
        ...
        next_action = lambda val: create_action(next_action_func)
    ...
    async def test_pipe_later_action_fails(self, action_context):
        ...
        initial_action = create_action(initial_action_func)
        failing_action = lambda val: create_action(failing_action_func)
    ...
    async def test_pipe_first_callable_error(self, action_context):
        ...
        with pytest.raises(ValueError, match="First item in pipe must be an Action, not a callable"):
