...
from silk.actions.input import (
    MouseMove, Click, DoubleClick, Fill, 
    Type, KeyPress, Drag, Select,
    MouseDown, MouseUp
)
from silk.models.browser import KeyModifier, ClickOptions, MouseMoveOptions
...
class TestMouseMove:
    @pytest.mark.asyncio
    async def test_mouse_move_to_coordinates(self, action_context, mock_browser_context):
        ...
        assert result.is_ok()
        mock_browser_context.mouse_move.assert_called_once_with(100, 200, MouseMoveOptions())
    
    @pytest.mark.asyncio
    async def test_mouse_move_to_element_handle(self, action_context, mock_element_handle, mock_browser_context):
    ...
    @pytest.mark.asyncio
    async def test_mouse_move_to_css_selector(self, action_context, mock_browser_page, mock_element_handle, mock_browser_context):
    ...
    @pytest.mark.asyncio
    async def test_mouse_move_with_selector_group(self, action_context, mock_browser_page, mock_element_handle, mock_browser_context):
    ...
    async def test_mouse_move_with_offsets(self, action_context, mock_browser_context):
        ...
        assert result.is_ok()
        mock_browser_context.mouse_move.assert_called_once_with(110, 220, MouseMoveOptions())
    
    @pytest.mark.asyncio
    async def test_mouse_move_selector_not_found(self, action_context, mock_browser_page):
        """Test mouse move when selector is not found."""
        # Setup the mock page to return error when queried
        mock_browser_page.query_selector.return_value = Error(Exception("Element not found"))
...
class TestClick:
    ...
    @pytest.mark.asyncio
    async def test_click_on_element_handle(self, action_context, mock_element_handle, mock_browser_context):
    ...
    @pytest.mark.asyncio
    async def test_click_with_custom_options(self, action_context, mock_browser_context):
        ...
        assert result.is_ok()
        mock_browser_context.mouse_click.assert_called_once_with("right", MouseMoveOptions(timeout=0))
    
    @pytest.mark.asyncio
    async def test_click_position_offset(self, action_context, mock_browser_context, mock_element_handle):
...
class TestDoubleClick:
    @pytest.mark.asyncio
    async def test_double_click_on_coordinates(self, action_context, mock_browser_context):
        ...
        assert mock_browser_context.mouse_click.call_count == 2 or mock_browser_context.mouse_double_click.call_count == 1
    
    @pytest.mark.asyncio
    async def test_double_click_with_custom_options(self, action_context, mock_browser_context):
...
class TestFill:
    ...
    async def test_fill_with_selector(self, action_context, mock_browser_page):
        ...
        assert result.is_ok()
        mock_browser_page.fill.assert_called_once_with("#input-field", "test text", 
                                                     action.options)
...
class TestDrag:
    @pytest.mark.asyncio
    async def test_drag_coordinates_to_coordinates(self, action_context, mock_browser_context):
        ...
        assert result.is_ok()
        mock_browser_context.mouse_drag.assert_called_once_with(source, target, action.options)
    
    @pytest.mark.asyncio
    async def test_drag_element_to_element(self, action_context, mock_browser_page, mock_element_handle, mock_browser_context):
...
class TestSelect:
    ...
    @pytest.mark.asyncio
    async def test_select_with_element_handle(self, action_context, mock_element_handle, mock_browser_page):
...
class TestMouseUpDown:
    ...
    async def test_mouse_down(self, action_context, mock_browser_context):
        ...
        assert result.is_ok()
        mock_browser_context.mouse_down.assert_called_once_with("left", MouseMoveOptions())
    ...
    async def test_mouse_up(self, action_context, mock_browser_context):
        ...
        assert result.is_ok()
        mock_browser_context.mouse_up.assert_called_once_with("right", MouseMoveOptions())