...
from silk.actions.context import CreateContext, SwitchContext, CreatePage, SwitchPage, CloseContext, ClosePage
...
class TestActionChaining:
    ...
    async def test_and_then_with_transform(self, action_context):
        ...
        chained_action = first_action.and_then(lambda val: ReturnValueAction(f"transformed_{val}"))
        result = await chained_action.execute(action_context)
    ...
    async def test_context_action_sequence(self, mock_browser_manager, action_context):
        ...
        mock_browser_manager.create_context = AsyncMock(return_value=Ok(create_context_result))
    ...
    async def test_error_handling_in_browser_sequence(self, action_context):
        ...
        action_context.get_page = AsyncMock(return_value=Error(Exception("Page not available")))
    ...
    @pytest.mark.asyncio
    async def test_mixed_return_types_in_sequence(self, action_context, mock_browser_page):
    ...
    @pytest.mark.asyncio
    async def test_create_and_switch_context(self, action_context, mock_browser_manager, mock_browser_context):
